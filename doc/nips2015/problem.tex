...
\begin{figure}[t]
 ...
 \includegraphics[width=0.49\textwidth,height=0.23\textheight,keepaspectratio]{figures/single-move.pdf}
  ...
  \caption{
    ...
    3 word phrase}
    \pl{left justify $x$ and write '$x_1$: on', '$x_2$: George', '$x_3$: str.', so that $q_i$'s match up;
    same with NONE, LOCATION, LOCATION}
    \pl{we should replace LNP with NLP!}
    \pl{also, I just utilities here are highly misleading;
      they should reflect the loss on the entire utterance having gotten $y_q = r$,
      not answering a specific query;
      let's replace them with $[0.8, 0.5, 0.8], [0.8, 0.8, 0.8], [0.8, 0.5, 0.8]$ (reverse positions 1 and 2 for the NLP ordering) (see text for why)
    }
  }
