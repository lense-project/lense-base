...
The successor state simply incorporates $q_k$: when the $q_k \in \{1, \ldots n\}$ then the issue time $s_k$ is set to $\now$, the response $r_k$ and response time $t_k$ are set to $\emptyset$.
If it is the crowd's turn (i.e.\ after the system chooses to wait), we sample the time of the first in-flight response along with its value. Let $F = \{1 \le j \le k-1 \given q_j \neq \varnothing \wedge r_j = \emptyset \}$. 
We sample response times conditioned on $t'_j > \now$, pick the smallest. Set $t_j \gets t'_j$.
When the system chooses to return an answer, then it receives a score determined by the utility at that point $U_{\now}()$.
