...
\begin{tabular}{l  r  r  r  r}
    ...
    \textbf{Entropic threshold - Bigrams} & 2059 ms & \textbf{1.44} & \textbf{93.80} \\ %\hline
...
When running experiments, we found that the results exhibited a high variance based on the current worker quality. %, fluctuating on the NER task between 87 and 96 F1, depending on workers.
...
\figureref{ner-running-f1} demonstrates that LENSE is able to consistently outperform other baselines.
This should not be surprising because we use crowd supervision.
However, as shown in \figureref{ner-running-f1}, we are able to steadily reduce the number of queries by a factor of 2 over time.
