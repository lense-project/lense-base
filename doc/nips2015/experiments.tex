...
\begin{enumerate}
  ...
  \item {\bf Threshold baseline}: Uses the following heuristic:
    For each label, $y_i$, the agent asks for $m$ queries such that $(1 - \p(y_i\given \bx))\times 0.3^m \ge 0.88$. % \footnote{We found $0.88$ to give the best results on our datasets.}
...
\begin{table}[t]
  ...
  {\bf NER (1000)}     & 
    ...
    We only consider the three entity tags corresponding to persons, locations or others\tablefootnote{%
    The original also includes the tags for organizations and miscellaneous, however the distinctions between these tags are technical, making it very difficult for non-expert crowd workers to provide accurate labels.}.
    &
...
We paid workers \$1.00 to join the retainer pool and an additional \$0.01 per query.
...
Neither LENSE nor the threshold needed more than 3 queries on a single label.
...
\begin{tabular}{l r r r r r | r r r r r}
  \multicolumn{6}{c|}{Named Entity Recognition} & 
    ...
    \\ \hline
    1-vote & 467 ms & 1.0 & TODO & TODO & 80.20
      & %1-vote & 
      1414 ms & 1.0 & 87.75 \\ %\hline
    3-vote & 750 ms & 3.0 & TODO & TODO & 85.41
        & %3-vote &
        1865 ms & 3.0 & 88.44 \\ %\hline
    5-vote & 1350 ms & 5.0 & TODO & TODO & 87.33 
        & 
        -- & -- & -- \\ \hline
    Online & n/a & n/a & TODO & TODO & 60.96
        % Online 
        & n/a & n/a & 87.43 \\    %\hline
    Threshold & \textbf{414 ms} & 0.61 & TODO & TODO & 88.27
        % threshold 
        & 1121 ms & 1.12 & \textbf{91.53} \\ %\hline
    \textbf{LENSE} & 724 ms & \textbf{0.44} & TODO & TODO & \textbf{88.45} 
    %\textbf{LENSE} 
...
For the latter two tasks, both on-the-job learning methods, threshold baseline and LENSE, perform comparably.
The simple heuristic adopted by the threshold baseline is sufficient to reason about expected marginals on both these tasks because they are single label prediction tasks which do not involve any interactions between labels.
...
\figureref{sentiment-tradeoff}, we show the number of queries per example on Sentiment with two different features sets, \textsc{bigrams} and \textsc{rnn} (as described in \tableref{dataset}).
