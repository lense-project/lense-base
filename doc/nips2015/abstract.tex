...
We propose a new paradigm that redistributes the same human effort to achieve better results faster: 
query humans in real time at test time to train a machine learning system {\em on the job}.
%\ac{is it clear what we're doing?}
%The system only queries humans when it is unsure of its output and learns to 
Querying humans introduces cost and latency but can improve accuracy over our model's prediction.
We explicitly manage these three key objectives by casting the problem as a Bayesian decision theoretic control problem
and draw on techniques from the game-playing literature.
The resulting live system provides high quality responses in real-time while starting {\em with no training data\/}: we maintain $> 90\%$ F1 consistently on a stream of test inputs on the three tasks studied at low cost and latency, despite noisy labels from crowd workers.
