...
Our algorithm (\algorithmref{mcvts})
that uses the UCT decision rule of MCTS but with a linear value approximation:
$\scvalue^f(s) = w \cdot \psi(s)$,
where $\psi$ are features of the state $s$ and $w$ are learned weights.
...
\caption{Monte Carlo tree search with value approximation}
\label{algo:mcvts}
  \begin{algorithmic}[1]
    \Function{monteCarloValue}{node $n$}
    \If{$n.\sctype = \scmin$}
      \State $n' \gets \argmin_{n' \in n.\scchildren} \frac{\scvalue^f(n')}{N(n')} + c \sqrt{\frac{2 \ln n.\scvisits}{n'.\scvisits}}$.
      \Comment Choose child using UCT
      \State $v \gets $ \Call{monteCarloValue}{$n'$}
      \State $\theta \gets \theta + \alpha (\scvalue^f(n') - v)\phi(n')$
      \Comment Update value approximation
      \State $n.\scvisits \gets n.\scvisits + 1, n'.\scvisits \gets n'.\scvisits + 1$
      \State \Return $v$
    \ElsIf {$n.\sctype = \scexpect$}
      \State $n' \sim n.\scchildren$
      \Comment Sample a child
      \State \Return \Call{monteCarloValue}{$n'$}
    \ElsIf {$n.\sctype = \scvalue$}
      \State \Return $n.\scvalue$
    \EndIf
    \EndFunction{}
