...
We now turn to finding an approximately optimal policy.
Our algorithm (\algorithmref{mcvts}) combines ideas from Monte Carlo tree search~\citep{kocsis2006bandit} to systematically explore the state space and TD-learning~\citep{sutton1988learning} to share statistics across game states.
Finally, we incorporate progressive widening~\citep{coulom2007computing} to deal with the challenge of a large state space that includes continuous variables (time).

\todo{Are more details about the algorithm required?}
...
\todo{Include progressive widening in algorithm box}.
